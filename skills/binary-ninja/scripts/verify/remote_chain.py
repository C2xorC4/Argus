...
# On Windows, Git Bash's /usr/bin/ssh has a libcrypto incompatibility with
# ED25519 keys generated by the Windows native OpenSSH agent.  Use the
# Windows-native binary explicitly so the mcp-binja key works from any shell.
_SSH_BIN = (r"C:\Windows\System32\OpenSSH\ssh.exe"
            if sys.platform == "win32" else "ssh")
_SCP_BIN = (r"C:\Windows\System32\OpenSSH\scp.exe"
            if sys.platform == "win32" else "scp")
